...
__global__ void sum_intensities_for_pixel(float *pixel, const dimensions panel_indices_dimensions, int *panel_indices) {
    ...
    const int pixel_coordinate = pixel_x * pixel_y_dimension + pixel_y;
 //   cudaBindTextureToArray(psf_texture, psf_array); // goes on host

    for(int panel_indexX = blockID.x - 1; panel_metadaaX =< blockID.x + 1 ; ++panel_indexX) {
      for(int panel_indexY = blockID.y - 1; panel_metadaaY =< blockID.y + 1 ; ++panel_indexY) {
       const int neighborhood_index = panel_index_lookup(panel_metadataX, panel_metadataY, panel_indices_dimensions);
       ...
       for (int star_index = panel_start; star_index < panel_end; ++star_index) {
         ...
         for (int color = 0 ; color < COLORS; ++color) 
            my_pixel +=  star_data.intensity[color] * cu_psf(star_data.x - pixel_x, star_data.y - pixel_y, color, psf_texture);
       }
     }
